package com.turpgames.doubleupex.objects.display;

import com.turpgames.framework.v0.component.Toast;
import com.turpgames.framework.v0.util.Color;
import com.turpgames.doubleupex.utils.DoubleUp;
import com.turpgames.doubleupex.utils.R;

public class DoubleUpToast {
	private final Toast toast;

	private final static DoubleUpToast instance = new DoubleUpToast();

	private DoubleUpToast() {
		toast = new Toast();
	}
	...
	public static void showWarning(String text, float fontScale, Toast.IListener listener) {
		show(text, fontScale, R.colors.ichiguBlue, listener);
	}
	...
	public static void show(String text, float fontScale, Color backColor, Toast.IListener listener) {
		String message = DoubleUp.getString(text);
		if (fontScale < 0)
	...
	public static DoubleUpToast preapreToast() {
		hide();
		reset();
		return instance;
	}
	...
	public DoubleUpToast setFontScale(float fontScale) {
		toast.setFontScale(fontScale);
		return this;
	}

	public DoubleUpToast setBackColor(Color backColor) {
		toast.setBackColor(backColor);
		return this;
	}

	public DoubleUpToast setTextColor(Color foreColor) {
		toast.setTextColor(foreColor);
		return this;
	}

	public DoubleUpToast setHideOnTap(boolean hideOnTap) {
		toast.setHideOnTap(hideOnTap);
		return this;
	}

	public DoubleUpToast setAlpha(float alpha) {
		toast.setAlpha(alpha);
		return this;
	}

	public DoubleUpToast setPadding(float padX, float padY) {
		toast.setPadX(padX);
		toast.setPadY(padY);
		return this;
	}

	public DoubleUpToast setSlideDuration(float slideDuration) {
		toast.setSlideDuration(slideDuration);
		return this;
	}

	public DoubleUpToast setDisplayDuration(float toastDuration) {
		toast.setDisplayDuration(toastDuration);
		return this;
	}

	public DoubleUpToast setMessage(String message) {
		toast.setMessage(message);
		return this;
	}

	public DoubleUpToast setListener(Toast.IListener listener) {
